"""Scraper for Sport1 (sport1.maariv.co.il)."""
...
class Sport1Parser(BaseSourceParser):
    """Scraper for Sport1 (sport1.maariv.co.il)."""
    source_name = "Sport1"
    source_code = "sport1"
    source_domain = "sport1.maariv.co.il"
    base_url = "https://sport1.maariv.co.il"
    discovery_urls = [
        "https://sport1.maariv.co.il",
        "https://sport1.maariv.co.il/israeli-soccer/",
        "https://sport1.maariv.co.il/world-soccer/",
        "https://sport1.maariv.co.il/basketball/",
    ]

    def is_valid_article_url(self, url: str) -> bool:
        if not super().is_valid_article_url(url):
            return False
        return bool(re.search(r"/article/\d+", url, re.I))

    def _extract_css_title(self, soup: BeautifulSoup) -> Optional[str]:
        for sel in ["h1.article-title", "h1.entry-title", "h1.title", "h1"]:
    ...
    def _extract_css_subtitle(self, soup: BeautifulSoup) -> Optional[str]:
        for sel in ["h2.article-subtitle", "div.article-sub-title", "h2.entry-subtitle", "h2"]:
            tag = soup.select_one(sel)
            if tag and clean_html_text(tag.text):
                return clean_html_text(tag.text)
        return None
    ...
    def _extract_css_author(self, soup: BeautifulSoup) -> Optional[str]:
        for sel in ["div.author-details .name", "span.author-name", "div.byline span", "span.author"]:
            tag = soup.select_one(sel)
            if tag and clean_html_text(tag.text):
                return clean_html_text(tag.text)
        return None
    ...
    def _extract_css_date(self, soup: BeautifulSoup) -> Optional[datetime]:
        for sel in ["time.entry-date", "span.publish-date", "div.date-time", "time"]:
            tag = soup.select_one(sel)
            if tag:
    ...
    def _extract_css_paragraphs(self, soup: BeautifulSoup) -> List[str]:
        paragraphs: List[str] = []
        for container_sel in ["div.article-body", "div.entry-content", "div.article-text", "article"]:
    ...
    def _extract_css_images(self, soup: BeautifulSoup, base_url: str) -> Tuple[Optional[ExtractedImage], List[ExtractedImage]]:
        ...
        main_img = soup.select_one("figure.featured-image img, div.main-image img, figure img")
        if main_img:
            src = main_img.get("src") or main_img.get("data-src")
            if src:
                caption_tag = soup.select_one("figcaption.image-caption, span.credit-text, span.photographer")
        ...
        for img in soup.select("div.gallery-images img, div.image-slider img"):
